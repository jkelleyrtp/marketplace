...
pub static Dashboard: Component<()> = |cx| {
    let user = state::use_current_user(&cx).unwrap();
...
static ResultsBrief: Component<()> = |cx| {
    let keywords = use_read(&cx, state::KEYWORDS);

    let rows = keywords.keys().copied().enumerate().map(|(idx, id)| {
        rsx!(Row {
            key: "{id}",
            id: id,
            is_gray: idx % 2 == 0,
            ...
            div { class: "container px-4 mx-auto",
                div { class: "py-4 bg-white rounded",
                    div { class: "flex px-6 pb-4 border-b",
                        h3 { class: "text-xl font-bold", "Searched Keywords" }
                    }
                    div { class: "pt-4 px-4 overflow-x-auto",
                        table { class: "table-auto w-full",
                            thead {
                                tr { class: "text-xs text-gray-500 text-left",
                                    th { class: "font-medium", "Keyword"}
                                    th { class: "font-medium", "Review Velocity"}
                                    th { class: "font-medium", "Freshness"}
                                    th { class: "font-medium", "Status" }
                                    th { class: "font-medium", "Action" }
                                }
                            }
                            tbody {
                                {rows}
...
fn Row(cx: Scope<RowProps>) -> Element {
    let state::KeywordEntry {
        ...
        keyword,
        products,
    } = use_keyword_entry(&cx, cx.props.id)?;
    let analysis = use_read(&cx, state::PRODUCT_ANALYSIS);

    let user = use_read(&cx, state::USERS).get(&creator).unwrap();
    ...
    let velo = products
        ...
        .map(|p| {
            //
            let analysis = analysis.get(&p.asin).unwrap();
            analysis.review_velocity
        })
        .sum::<f64>()
        / products.len() as f64;
    ...
    cx.render(rsx!(
        tr { class: format_args!("text-xs {}", if cx.props.is_gray {"bg-gray-50"} else {""}),
            td { class: "flex px-4 py-3",
                img { class: "w-8 h-8 mr-4 object-cover rounded-md", src: "{img_url}" }
                div {
                    p { class: "font-medium", "{keyword}" }
                    p { class: "text-gray-500", "{user.short_name}" }
                }
            }
            td { class: "font-medium", {[format_args!("{}", (velo as i64).to_formatted_string(&loc))]} }
            td { class: "font-medium", "500" }
            td {
...
static QuickActions: Component<()> = |cx| {
    let set_route = use_set(&cx, state::ROUTE);
                            ...
                            div { class: "pt-6 px-6 mb-8",
                                div { class: "flex mb-8 justify-between items-center",
                                    a { class: "py-2 px-3 bg-indigo-500 hover:bg-indigo-600 rounded text-xs text-white",
                                        href: "#",
                                        onclick: move |_| set_route(AppRoute::Search),
                                        "Search"
                                    }
                                }
                                div {
                                    ...
                                    p { class: "text-sm text-gray-500",
                                        "Use the Amazon Scraper and Helium10 API to find new business opportunities."
                                    }
                                }
                            }
                        }
                        ...
                        div { class: "bg-white shadow rounded overflow-hidden",
                            div { class: "pt-6 px-6 mb-8",
                                div { class: "flex mb-8 justify-between items-center",
                                    a { class: "py-2 px-3 bg-indigo-500 hover:bg-indigo-600 rounded text-xs text-white",
                                        href: "#",
                                        onclick: move |_| set_route(AppRoute::Review),
                                        "Review"
                                    }
                                }
                                div {
                                    h3 { class: "mb-2 text-xl font-bold", "Review all products" }
                                    p { class: "text-sm text-gray-500", "Review the analysis of products you're tracking." }
                                }
                            }
                        }
                    }
